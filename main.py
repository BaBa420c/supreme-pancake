...
# Bot configuration
TOKEN = "6648854394:AAFbLk3j_F-2DNXD_nixFW5ugS3Oem4jjw0"  # Replace with your bot token
OWNER_ID = (1481105004)  # Replace with your owner ID
...
def start(message):
    ...
    if user_id in load_banned_users():
        bot.reply_to(message, "ɴɪɢɢᴀ🖕")
        return
    if user_id not in load_approved_users():
        bot.reply_to(message, "𝘠𝘰𝘶 𝘢𝘳𝘦 𝘯𝘰𝘵 𝘢𝘱𝘱𝘳𝘰𝘷𝘦𝘥 𝘵𝘰 𝘶𝘴𝘦 𝘵𝘩𝘪𝘴 𝘣𝘰𝘵. 𝘊𝘰𝘯𝘵𝘢𝘤𝘵 𝘵𝘩𝘦 𝘰𝘸𝘯𝘦𝘳- @scammerverified")
        return
...
def add_user(message):
    if message.from_user.id != OWNER_ID:
        bot.reply_to(message, "ᴅᴜᴍʙ ɴɪɢɢᴀ💀")
        return
    try:
        ...
        bot.reply_to(message, "ρяσνι∂є α υѕєя ι∂ тσ αρρяσνє.")
...
def ban_user_command(message):
    ...
    try:
        ...
        bot.reply_to(message, "ρяσνι∂є α υѕєя ι∂ тσ вαη..")
...
def handle_document(message):
    ...
    if user_id in load_banned_users():
        ...
        return
    if user_id not in load_approved_users():
        bot.reply_to(message, "𝘊𝘰𝘯𝘵𝘢𝘤𝘵 𝘵𝘩𝘦 𝘰𝘸𝘯𝘦𝘳- @scammerverified")
        return
